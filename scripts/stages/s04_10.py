...
from stages.util import CONFIG, DATA, done, read_jsonl, run_dir, write_jsonl
...
def audit_hit(name: str, rx: str, text: str) -> bool:
    ...
    if name == "isos_label":
        # Delegated to protocol.isos_in_text, NEVER re-implemented here.
        # AUDIT_RX["isos_label"] is a raw ASCII-hyphen pattern, so it missed
        # every publisher that types U+2010: measured against the R1 human
        # labels it found 8 of 16 ISOS papers (recall 0.50). Three distinct
        # causes, all in one regex -- Unicode hyphens (6 papers), the O
        # (outdoor) family absent from [LVDTP], and the LC/LT compound
        # families. Handbook 4.4 rule 1: normalise Unicode BEFORE matching.
        #
        # protocol.py already owned this vocabulary for the figure axis. A
        # second copy here would be the divergent-guard trap that let a
        # narration leak reach a shipped PDF through the narrowest of three
        # copies, so the canonical scanner is imported instead.
        from stages.protocol import isos_in_text
        return isos_in_text(text)
    if name != "area_stated":
...
def gates() -> dict:
    return yaml.safe_load((CONFIG / "gates.yaml").read_text(encoding="utf-8"))


# ---------------------------------------------------------------- 04_venues
def venues(month: str) -> dict:
    rd = run_dir(month)
    ...
    (rd / "04_venue_percentiles.json").write_text(json.dumps(
        {"n_reference_venues": n_ref, "snapshot_date": month,
...
def subset(month: str) -> dict:
    ...
    # month 1: history empty -> novelty_flag 1.0 for all (5.8 empty-history rule)
    ...
    # force-includes: month 1 has no card history -> structurally empty (5.8)
    (rd / "08_force_include_pending.json").write_text(json.dumps(
        {"pending": [], "reason": "month 1: empty card history, no cumulative max PCE"},
...
def stats(month: str) -> dict:
    ...
    for ax in ("composition", "defects", "interfaces", "architecture", "stability", "scale_up"):
        ...
        S[f"axes.{ax}.z_vs_6mo"] = None          # <3 months history
        S[f"axes.{ax}.delta_vs_prev_month_pp"] = None   # month 1 (Q46)
        S[f"axes.{ax}.delta_basis"] = "month 1: no prior month"
    ...
    (hist / f"{month}.json").write_text(json.dumps(S, indent=2), encoding="utf-8")
