"""20_chemrxiv: the ChemRxiv preprint submission package (Q66, revised Q68).

WHY THIS IS NOT THE arXiv STAGE ANY MORE
----------------------------------------
Havid changed the target on 2026-09-09: this review series goes to ChemRxiv,
not arXiv, because the topic is chemistry-scoped. This file replaces the old
`s20_arxiv.py` (chemrxiv supersedes it), which built exactly what arXiv wants
-- a self-contained tarball of .tex + .bbl + relative-path figures, plus
`00README.XXX` and archive codes like `physics.app-ph`.

ChemRxiv wants something different, so this is a format change and not a
find-and-replace:
  * the MAIN UPLOAD IS THE RENDERED PDF (the portal takes PDF or .docx by
    drag-and-drop; it does not compile LaTeX for you). So the deliverable the
    human actually submits is `manuscript_v4.pdf`, and the checklist is
    generated around it.
  * it has SUBJECT CATEGORIES ("Materials Science", "Energy"), not arXiv
    archive codes. `primary_category: physics.app-ph` would be meaningless.
  * a posted preprint gets a DOI under prefix 10.26434. We NEVER write that
    DOI from a script -- it does not exist until posting, and inventing one
    would be a fabricated identifier. The field stays null and the checklist
    asks the human to paste it back.

The .tex tarball is STILL BUILT, deliberately, but its role changed: it is an
archival/journal-transfer artifact, not the submission payload. Dropping it
would lose the standalone-compile proof that has caught real preamble bugs.

Everything about the platform is read from config/preprint.yaml. Nothing here
hardcodes the word ChemRxiv in a way that a future move would have to hunt
down again, which is what made this migration a 9-file edit.

NOTE ON THE WORD "arxiv" ELSEWHERE: arXiv remains a HARVEST SOURCE (stage 01
queries it, the ledger has an arxiv_id column, cited works carry arXiv DOIs).
That is unrelated to where we submit and must not be renamed.

Run: python scripts/stages/s20_chemrxiv.py 2026-07 [v4]
"""
...
from stages.util import CONFIG, ROOT, done, read_jsonl, run_dir
# The corresponding-author email has ONE definition. s18d prints it on the
...
def build(month: str) -> dict:
    ...
    rd = run_dir(month)
    P = yaml.safe_load((CONFIG / "preprint.yaml").read_text(encoding="utf-8"))
    ...
    # Archival artifact now, not the upload. Kept because the standalone
    # compile below is a real proof that the preamble is portable.
    ...
    # Keep the preamble pdflatex-portable. This mattered for arXiv's compile
    # farm and still matters for a journal transfer, so it stays.
    ...
    # ---------------- 4. the PDF the human actually uploads --------------
    # ChemRxiv takes the main content as a PDF. Fail closed if it is missing:
    # a submission package whose main file is absent is not a package, and the
    # old stage could not detect this because its payload was the tarball.
    main_pdf = mdir / f"manuscript_{ver}.pdf"
    if not main_pdf.exists():
        raise SystemExit(
            f"FAIL-CLOSED: {P['platform']} takes the PDF as the main upload "
            f"and {main_pdf} does not exist. Run s18d before stage 20.")
    shutil.copy(main_pdf, out / main_pdf.name)
    si_pdf = mdir / f"supplementary_{ver}.pdf"
    if si_pdf.exists():
        shutil.copy(si_pdf, out / si_pdf.name)

    # ---------------- 5. submission metadata (platform-shaped) -----------
    title = pack_title
    m_title = re.search(r"\{\\LARGE\\bfseries (.+?)\\par\}", body)
    if m_title:
        title = re.sub(r"\\[a-zA-Z]+\{?|\}", "", m_title.group(1)).strip()

    # Abstract, taken from the built manuscript rather than retyped, so the
    # portal paste box and the PDF cannot disagree.
    abstract = ""
    try:
        mtxt = md.read_text(encoding="utf-8")
        ma = re.search(r"##\s*Abstract\s*\n+(.+?)(?=\n##\s|\n\*\*Keywords)",
                       mtxt, re.S)
        if ma:
        ...
        abstract = ""
    if not abstract:
        raise SystemExit("FAIL-CLOSED: could not read the abstract out of "
                         f"{md.name}; the portal paste box would be empty.")

    meta = {
        "platform": P["platform"],
        "platform_url": P["platform_url"],
        "submission_route": P["submission_route"],
        "api_submission_supported": P["api_submission_supported"],
        "title": title,
        "abstract": abstract,
        "authors": [
            {"name": "Havid Aqoma",
             ...
             "orcid": "0000-0003-1264-1916",
             "affiliations": list(range(1, len(AFFIL) + 1))},
        ],
        "affiliations": {str(i): a for i, a in enumerate(AFFIL, 1)},
        "corresponding_email": CORRESP_EMAIL,
        "subject_category_primary": P["subject_category_primary"],
        "subject_categories_secondary": P["subject_categories_secondary"],
        "license": P["license"],
        # Assigned on posting. Never written by a script (see module docstring).
        "preprint_doi": P["preprint_doi"],
        "doi_prefix_if_assigned": P["doi_prefix"],
        "main_content_file": main_pdf.name,
        "supplementary_files": ([si_pdf.name] if si_pdf.exists() else []),
        "funding_statement": ACK,
    ...
    (out / "submission_metadata.json").write_text(json.dumps(meta, indent=2),
                                                  encoding="utf-8")

    # A generated checklist, because the submission is a MANUAL portal upload
    # and the steps must not live in someone's memory.
    (out / "SUBMISSION_CHECKLIST.md").write_text(
        f"# {P['platform']} submission checklist\n\n"
        f"Generated {time.strftime('%Y-%m-%d %H:%M')} from run `{rd.name}`.\n"
        f"Route: **{P['submission_route']}** at {P['platform_url']} -- there is "
        f"no automated submission code path in this repo, by design.\n\n"
        f"## Upload\n\n"
        f"1. Main content: `{main_pdf.name}` (ChemRxiv accepts PDF or .docx; "
        f"the PDF is the built artifact and is gate-checked).\n"
        f"2. Supplementary: "
        f"{'`' + si_pdf.name + '`' if si_pdf.exists() else '(none built)'}\n"
        f"3. Data pack: attach `data/` or link the repository release.\n\n"
        f"## Paste-ready fields\n\n"
        f"- **Title:** {title}\n"
        f"- **Corresponding author:** Havid Aqoma, {CORRESP_EMAIL}\n"
        f"- **Primary subject category:** {P['subject_category_primary']}\n"
        f"- **Secondary categories:** "
        f"{', '.join(P['subject_categories_secondary'])}\n"
        f"- **Licence:** {P['license']}\n"
        f"- **Abstract:** see `submission_metadata.json` (copied verbatim from "
        f"the built manuscript, so it cannot drift from the PDF)\n\n"
        f"## After posting\n\n"
        f"- ChemRxiv assigns a DOI under `{P['doi_prefix']}`. Paste it into "
        f"`config/preprint.yaml` as `preprint_doi`. Do not let any script "
        f"invent this value before it exists.\n\n"
        f"## Archival, not uploaded\n\n"
        f"- `manuscript.tex` + `manuscript.bbl` + `fig/` and the tarball are "
        f"kept for a later journal transfer. ChemRxiv does not compile LaTeX.\n",
        encoding="utf-8")

    # ---------------- 6. archival tarball ----------------
    tgz = mdir / f"{P['platform_slug']}_{month}.tar.gz"
    with tarfile.open(tgz, "w:gz") as tf:
        for f in ("manuscript.tex", "submission_metadata.json",
                  "SUBMISSION_CHECKLIST.md", main_pdf.name):
            if (out / f).exists():
                tf.add(out / f, arcname=f)
        if si_pdf.exists() and (out / si_pdf.name).exists():
            tf.add(out / si_pdf.name, arcname=si_pdf.name)
    ...
    # The migration guard. The package must not tell a reader it is for arXiv.
    # Checked on the GENERATED text, because the whole failure mode here is a
    # stale platform string surviving in an artifact nobody reads.
    stale = []
    for f in (out / "submission_metadata.json", out / "SUBMISSION_CHECKLIST.md"):
        if f.exists() and re.search(r"arxiv", f.read_text(encoding="utf-8"), re.I):
            stale.append(f.name)

    meta_out = {"platform": P["platform"],
                "package_dir": str(out),
                "main_content_pdf": str(out / main_pdf.name),
                "supplementary_pdf": (str(out / si_pdf.name)
                                      if si_pdf.exists() else None),
                "tex": str(tex), "tex_compiles": compiled,
                "tarball": str(tgz), "tarball_bytes": tgz.stat().st_size,
                "tarball_members": names,
                "absolute_paths_in_tar": absolute,
                "stale_platform_refs": stale,
                "checklist": str(out / "SUBMISSION_CHECKLIST.md"),
                "metadata": str(out / "submission_metadata.json"),
                "csv_files": sorted(f.name for f in data.glob("*.csv")),
                "data_dir": str(data)}
    if absolute:
        raise SystemExit(f"FAIL-CLOSED: absolute paths in tarball {absolute}")
    if stale:
        raise SystemExit(f"FAIL-CLOSED: submission package still references "
                         f"arXiv in {stale}; the target is {P['platform']}")
    done(rd, "20_chemrxiv", **meta_out)
