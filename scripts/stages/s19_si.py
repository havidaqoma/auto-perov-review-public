...
def build(month: str) -> dict:
    ...
    # through G4.
    #
    # The counter then went blind the other way: from v4 on the build emits
    # [\href{https://doi.org/...}{12}], and a plain-[12] count found none, so
    # every v4/v5 SI printed "Cited in the main text | 0" beside 75-184 linked
    # citations. The one counter now lives in stages/si_facts.py and counts
    # both marker forms with the same guard. Zero with a non-empty reference
    # list is not a manuscript this pipeline can produce, so it fails closed.
    from stages.si_facts import (manuscript_date, manuscript_title,
                                 measured_citations)
    _n_refs, n_cited_main = measured_citations(mtext)
    if _n_refs and not n_cited_main:
        raise SystemExit(
            f"FAIL-CLOSED: manuscript_{ver}.md lists {_n_refs} references but "
            f"0 were found cited in its body; the citation counter cannot see "
            f"this build's marker form.")
    ...
    #
    # Deriving it was still a second path that could diverge: only ver "v4"
    # derived, every other version fell to the v3 literal, which is how the
    # 2026-H1 v6 SI named July's issue. The SI describes ONE built manuscript,
    # so title and date are now read from that manuscript's own title block.
    # The v3 literal stays only for pre-v4 manuscripts that have no block.
    title_line = manuscript_title(mtext)
    if title_line is None:
        if ver not in ("v2", "v3"):
            raise SystemExit(
                f"FAIL-CLOSED: no \\LARGE\\bfseries title block in "
                f"manuscript_{ver}.md; the SI cannot name the paper it "
                f"supports.")
    ...
    ms_date = manuscript_date(mtext) or (
        f"{time.strftime('%B ')}{int(time.strftime('%d'))}"
        f"{time.strftime(', %Y')}")
    ...
    md = [
        ...
        "must appear inside its own quotation, and third, the quotation is "
        "truncated to 25 words before that check rather than after it, so a value can never "
        ...
        "across six fixed axes, with the title weighted twice, and mechanism "
        ...
        "first, and each DOI is a live link.",
        "", "\n".join(t5), "",
        "## Note S6. Limitations",
        "", limits, "",
    ...
    # Figure S1 is drawn HERE, from the numbers in Table S2, not by the figure
    # stage from stats.json: stats' selection.n_cited is the pre-drafting cap
    # (min(depth, max_body_citations)), so July's "Cited in body" bar read 180
    # for a manuscript citing 79. One source for the table and the figure.
    from stages.si_facts import draw_funnel
    _funnel = figdir / "S2_corpus_funnel.pdf"
    draw_funnel(_funnel,
                ["Passed the scope gate", "Usable abstract",
                 "Depth tier (close reading)", "Verified extraction record",
                 "Cited in the main text"],
                [S["corpus.n"], S["selection.n_eligible"],
                 S["selection.n_depth"], len(cards), n_cited_main])
    if (outdir / "fig" / _funnel.name).exists():   # edition's shipped copy
        shutil.copy(_funnel, outdir / "fig" / _funnel.name)
...
def render_pdf(f: pathlib.Path, pdf: pathlib.Path, rd: pathlib.Path) -> None:
    """The monthly SI render (pandoc + tectonic, 10pt). One definition, so a
    repair of a shipped v4 SI renders exactly as the original build did."""
