...
def _normalise(text: str) -> str:
    ...
    # GREEK SMALL LETTER MU -> MICRO SIGN. Visually identical, different
    # codepoints, and only U+00B5 is in the unit table. A writer emitted
    # "2 \u03bcm" (U+03BC); the converter left it bare because it matched no
    # unit key, and tectonic then failed the whole build:
    #
    #   Missing character: There is no \u03bc (U+03BC) in font lmroman10-regular
    #   FAIL-CLOSED: pandoc rc=1
    #
    # Every one of the 20 gates had already passed. The manuscript died at
    # the render step on ONE character, which is the same lesson as the
    # U+2212 minus sign one line above: normalise BEFORE matching, or an
    # identical-looking glyph silently misses the table.
    text = text.replace("\u03bc", "\u00b5")               # Greek mu
...
_UNITS = {
    ...
    # uW/nW were ABSENT and that is why "100 uW per cm2" reached the page
    # flat. A missing unit does not degrade gracefully: the unknown token
    # broke the whole run, so the neighbouring cm2 stayed flat too and G9a
    # reported the NEIGHBOUR rather than the cause.
    "mW": r"\milli\watt", "kW": r"\kilo\watt", "W": r"\watt",
    "uW": r"\micro\watt", "\u00b5W": r"\micro\watt", "nW": r"\nano\watt",
...
# A unit run may be joined by whitespace ("cm2 V-1 s-1"), by a SLASH
# ("mA/cm2", "W/cm2"), or by the word "per" ("100 uW per cm2"). Only the
# whitespace form was accepted, and the trailing (?![\w\-/]) lookahead then
# made the slash form actively FAIL: in "1.5 mA/cm2" the run matched "mA",
# the lookahead saw "/", and the WHOLE quantity was rejected. The unit
# reached the page flat and G9a reported "unit flat exponent: cm2" against
# prose that was perfectly correct.
#
# This is the missing-unit lesson one level up. The handbook records that a
# missing UNIT drops every unit beside it in the run; a missing JOINER does
# the same thing. Three real survivors on the 2025 build:
#   "1.5 mA/cm2"   "100 uW per cm2"   "20 W/cm2"
#
# A slash or "per" between two unit tokens means divide, so every token
# after such a joiner takes a negative exponent. The joiners are therefore
# kept in the match so _qty_sub can see which tokens are divided.
_UJOIN = r"(?:\s*/\s*|\s+per\s+|\s+)"
_UNITRUN = (rf"(?:{_UALT})(?:-?[1-9])?"
            rf"(?:{_UJOIN}(?:{_UALT})(?:-?[1-9])?)*")
# The lookahead no longer forbids "/": a slash INSIDE a run is legal now, and
# a trailing slash cannot occur because a run must end on a unit token.
...
def _split_run(run: str) -> list[tuple[str, str | None, bool]]:
    """Split a unit run into (token, exponent, is_divided) triples.

    `is_divided` is True for every token appearing after a "/" or a "per",
    because both mean the same thing and both must invert the exponent.
    Splitting explicitly, rather than stripping the joiners first, is what
    lets "mA/cm2" and "cm2 V-1 s-1" share one code path: the first divides,
    the second does not, and the token alone cannot tell you which.
    """
    out: list[tuple[str, str | None, bool]] = []
    divided = False
    s = re.sub(r"\s+", " ", run.strip())
    pos = 0
    while pos < len(s):
        if out:
            mj = re.match(r"\s*(?:/|per)\s*", s[pos:])
            if mj:
                divided = True
                pos += mj.end()
                continue
        if s[pos] == " ":
            pos += 1
            continue
        mt = _UTOK.match(s, pos)
        if not mt or not mt.group(1):
            break
        out.append((mt.group(1), mt.group(2), divided))
        pos = mt.end()
    return out
...
def _qty_sub(m: re.Match) -> str:
    ...
    parts = []
    for tok, exp, divided in _split_run(m.group(2)):
        if divided:
            # "/cm2" is cm to the MINUS two. An exponent that is already
            # negative is left alone: "W/cm-2" would be a double negative,
            # which no writer intends.
            if not exp:
                exp = "-1"
            elif not exp.startswith("-"):
                exp = "-" + exp
        parts.append(_unit_expr(tok, exp))
    if not parts:
        return m.group(0)
