...
if __name__ == "__main__":
    # No default period, for the same reason as the other stages: a bare run
    # would harvest a 2026-07 run dir this repo does not own. This stage is
    # the MONTHLY harvest, kept for single-month dry runs; the annual sweep
    # lives in s01_harvest_yearly.py.
    if len(sys.argv) < 2:
        raise SystemExit(
            "usage: s01_harvest.py <YYYY-MM>   e.g. 2025-06\n"
            "No default: the period must be stated, never inferred.\n"
            "For a whole year use s01_harvest_yearly.py <YYYY>.")
    harvest(sys.argv[1])
