...
def provenance(rd: pathlib.Path, cards: list, models: dict) -> dict:
    ...
    if len(ext) > 1:
        raise SystemExit(f"FAIL-CLOSED provenance: cards were extracted by "
                         f"MORE THAN ONE model {dict(ext)}. A single issue must "
                         f"declare one extractor; re-extract the whole month.")
...
def build(month: str) -> dict:
    ...
    for i, a in enumerate(AFFIL, 1):
        sep = r"\\[2.0em]" if i == len(AFFIL) else r"\\[0.25em]"
