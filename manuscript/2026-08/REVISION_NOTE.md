...
- `verify_notation_pdf.py` : August main 52 correct, 0 flat (July main 48
  correct, 0 flat). Both SIs carry no notation tokens.
- Page counts: August 14pp main (unchanged), 6pp SI (was 5). The SI gained a
  page because merged sentences reflow.
...
## One August-specific note

The August main text was the worst document in this batch at 36.3% of
sentences over 35 words. The first rewrite pass over-corrected hard, landing
at 18.3 mean and 0.5% over 35 words, which is as wrong as the original: real
papers vary sentence length. 32 of 77 splits were then reverted to restore
long sentences, and the reverting itself found a real bug. Some splits are
PAIRED, where one edit reinserts a citation that its partner removes from a
merged bracket. Reverting one half of such a pair silently dropped citations
1, 32 and 42. That was caught by the citation contract, not by reading, which
is the whole reason the contract is checked mechanically against the v4
original rather than trusted from a summary.
...
- `gate_report.json` now exists. It was produced by
  ...
  without rebuilding, so the hand-revised prose is untouched. 17 of 18 build gates pass. G8-novelty fails: one 12-word framing sentence is shared with the final July text. Every per-section similarity ratio is far under its 0.25 bound. The August v4 build report carried the same failing verdict, and it ships as a recorded failure rather than a widened gate.
- `scripts/verify_pdf.py 2026-08 v5` runs against that report: 10 of 11
  checks pass and the eleventh reports the recorded G8-novelty failure.
- `chemrxiv/` and `chemrxiv_2026-08.tar.gz` were built from the v5 PDFs by
